...
If(-not [string]::IsNullOrEmpty($StandAlonePrimarySite))
{
	...
	## Set first column format
	SetWordCellFormat -Collection $Table.Rows.Item(1).Cells -Bold -BackgroundColor $wdColorGray15 -Color;
...
{  
	...
	ForEach($CMDistributionPoint in $CMDistributionPoints)
	{
		...
		WriteWordLine 3 0 "$($CMDPServerName)" <# LINE THAT PRINTS DP NAME #>
		...
		Else
		{
			$CMDPDrives = (Get-WmiObject -Class SMS_DistributionPointDriveInfo `
				...
				-ComputerName $SMSProvider).Where({$_.NALPath -like "*$CMDPServerName*"})
            $DriveCount = $CMDPDrives | measure
            $DPRoleWordTable.Add("Drive Count", $DriveCount.Count)
			...
			ForEach($CMDPDrive in $CMDPDrives)
			{
                ...
                $DPDiskWordTable.Add("Drive Partition","$($CMDPDrive.Drive):")
                $DPDiskWordTable.Add("Total Space", "$([MATH]::Round($Size,2)) GB")
                $DPDiskWordTable.Add("Free Space", "$([MATH]::Round($Freesize,2)) GB")
                $DPDiskWordTable.Add("Percent Free", "$($CMDPDrive.PercentFree)%")
			}
			...
			try 
			{
                ...
                $DPRoleWordTable.Add("Memory", "$TotalMemory GB")
			}
			catch 
			{
				WriteWordLine 0 3 "Unknown" 
			}	
		}
		...
		Switch ($IsPXE)
		{
			1 
				{
                    $DPRoleWordTable.Add('PXE', "Enabled")
					Switch ($UnknownMachines)
					{
						1 {
                            $DPRoleWordTable.Add('Unknown Machine Support', "Yes")
                        }
						0 {
                            $DPRoleWordTable.Add('Unknown Machine Support', "No") 
                        }
					}
				}
			0
				{
                    $DPRoleWordTable.Add('PXE', "Disabled")
				}
		}
		...
		If(-not [string]::IsNullOrEmpty($DPGroupIDs))
		{
			...
			ForEach($DPGroupID in $DPGroupIDs)
			{
                ...
                $DPGroupNames += "$DPGroupName,"
			}

            $DPRoleWordTable.Add("DP Groups", $DPGroupNames)
		}
		Else
		{
            $DPRoleWordTable.Add("DP Groups", "None")
		}

	    WriteWordLine 4 1 "DP Information"        
        $Table = AddWordTable -Hashtable $DPRoleWordTable -Format -155 -AutoFit $wdAutoFitContent
        ...
        WriteWordLine 4 1 "DP Disk Information" 
        $DiskTable = AddWordTable -Hashtable $DPDiskWordTable -Format -155 -AutoFit $wdAutoFitContent

	    ## Set first column format
	    SetWordCellFormat -Collection $DiskTable.Rows.Item(1).Cells -Bold -BackgroundColor $wdColorGray15 -Underline -Italic
...
If($ListAllInformation)
{
    $DevCollectionHashArr = @{}
	ForEach($DeviceCollection in $DeviceCollections)
	{
		$DevColHashRow = @{'Collection Name'= $DeviceCollection.Name; `
		'Collection ID' = $DeviceCollection.CollectionID; `
		'Total count of members' = $DeviceCollection.MemberCount;
        'Limited to Device Collection' = $DeviceCollection.LimitToCollectionName};
		$DevCollectionHashArr += $DevColHashRow;
    }
		
    $Table = AddWordTable -Hashtable $DevCollectionHashArr `
	-Columns 'Collection Name', 'Collection ID', 'Total members', 'Ltd. to Device Collection' `
	-Headers 'Collection Name', 'Collection ID', 'Total members', 'Ltd. to Device Collection' `
	-Format -155 `
	...
	$Table.Columns.Item(4).Width = 300;
